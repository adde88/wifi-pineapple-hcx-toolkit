...
### Added
- **System Resilience**: The installer now creates the `/etc/hcxtools/VERSION` file, ensuring the new versioning system works correctly even on fresh installations.

## [6.6.0] - 2025-07-11
### Changed
- **Major Toolchain Upgrade**: The toolkit has been completely overhauled to support **`hcxdumptool` v6.3.5** and the new **`hcxlabtool`** suite. This modernizes the core engine for better performance and future compatibility.
- **New GPS Architecture**: GPS functionality in the launcher (`--enable-gps`) no longer uses the deprecated internal `hcxdumptool` GPS feature. It now launches the new, dedicated `hcxnmealog` binary in the background for more stable and reliable NMEA data logging.
- **Updated Command-Line Arguments**: All internal launcher commands have been updated to use the modern flags and syntax required by `hcxdumptool` v6.3.5 (e.g., `--rcascan=p` for passive mode).
- **Standardized Interface**: All documentation, examples, and help text have been updated to use `wlan2` as the default wireless interface, reflecting the standard for the WiFi Pineapple MKVII.

### Added
- **Dedicated Geotracking Analysis**: The `hcx-analyzer.sh` script now has a fully rewritten `--mode geotrack` which correctly finds and processes `.nmea` log files generated by `hcxnmealog`. This allows for reliable conversion to KML map files.

### Fixed
- **TUI Visibility Restored**: Fixed a critical bug where the `hcxdumptool` real-time status display (TUI) would not appear, causing the script to look like it was hanging. The launcher now correctly handles process I/O, ensuring the status screen is always visible in interactive mode.
- **Automatic Post-Capture Analysis**: Corrected a bug where the `RUN_ANALYSIS` setting in `/etc/hcxtools/hcxscript.conf` was being ignored. The launcher now correctly loads this configuration on startup, restoring the automatic analysis workflow.
